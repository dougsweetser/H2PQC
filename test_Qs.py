...
def test__1471_Lorentz_next_rotation():
    ...
    assert next_rotation.t == Q1.t
    rot = rotation_and_or_boost(q2244, vector_q(next_rotation))
    assert math.isclose(rot.t, 2)
    assert math.isclose(square(rot).t, square(q2244).t)
    next_rotation = Lorentz_next_rotation(Q1, Q1)
    assert equal(next_rotation, Q1)


def test__1471_Lorentz_next_randomized_rotation():
    with pytest.raises(ValueError):
        Lorentz_next_randomized_rotation(Q1, q4321)
    next_randomized_rotation = Lorentz_next_randomized_rotation(Q1, q1324)
    print("next_rotation: ", next_randomized_rotation)
    assert next_randomized_rotation.t == Q1.t
    rot = rotation_and_or_boost(q2244, vector_q(next_randomized_rotation))
    assert math.isclose(rot.t, 2)
    assert math.isclose(square(rot).t, square(q2244).t)
    next_rotation = Lorentz_next_rotation(Q1, Q1)
    assert equal(next_rotation, Q1)
